##
## Growatt CAN protocol decoder for PulseView/libsigrokdecode.
##
## This decoder is standalone on a logic CAN RX signal. It reuses the built-in
## CAN decoder and adds Growatt-specific annotations for decoded frames. It can
...
try:
    from .growatt_can import FRAME_NAMES, describe_packet, format_data, frame_summary
except Exception:
    from growatt_can import FRAME_NAMES, describe_packet, format_data, frame_summary
...
class Ann:
    GROWATT_FRAME = len(CanDecoder.annotations) if CanDecoder is not None else 0
    GROWATT_PAYLOAD = GROWATT_FRAME + 1
    GROWATT_DECODED = GROWATT_FRAME + 2
    GROWATT_WARNING = GROWATT_FRAME + 3
...
class Decoder(BaseDecoder):
    ...
    id = 'growatt_can'
    name = 'Growatt CAN'
    longname = 'Growatt-compatible CAN'
    desc = 'Growatt low-voltage BMS/inverter frames over Classic CAN.'
    license = 'gplv2+'
    inputs = ['logic']
    outputs = ['growatt_can']
    ...
    if CanDecoder is not None:
        annotations = CanDecoder.annotations + (
            ('growatt-frame', 'Growatt frame'),
            ('growatt-payload', 'Growatt payload'),
            ('growatt-decoded', 'Growatt decoded value'),
            ('growatt-warning', 'Growatt warning'),
        )
        annotation_rows = CanDecoder.annotation_rows + (
            ('growatt-frames', 'Growatt Frames', (Ann.GROWATT_FRAME,)),
            ('growatt-payloads', 'Growatt Payload', (Ann.GROWATT_PAYLOAD,)),
            ('growatt-decoded-values', 'Growatt Decoded', (Ann.GROWATT_DECODED,)),
            ('growatt-warnings', 'Growatt Warnings', (Ann.GROWATT_WARNING,)),
        )
    else:
        annotations = (
            ('growatt-warning', 'Growatt warning'),
    ...
    def put_growatt_ann(self, ss, es, ann, texts):
    ...
    def putpy(self, data):
        ...
        if rtr_type not in (None, 'Data', 'data', 0):
            self.put_growatt_ann(self.ss_packet, self.es_packet, Ann.GROWATT_WARNING,
                                 ['Remote frame 0x{:03X} ignored'.format(can_id), 'RTR ignored'])
            return

        packet = (frame_type, can_id, rtr_type, dlc, payload)
        self.put_growatt_ann(self.ss_packet, self.es_packet, Ann.GROWATT_FRAME,
                             [frame_summary(packet)])
        self.put_growatt_ann(self.ss_packet, self.es_packet, Ann.GROWATT_PAYLOAD,
                             [format_data(payload), 'DATA'])
        self.put_growatt_ann(self.ss_packet, self.es_packet, Ann.GROWATT_DECODED,
