##
## WOW / JK 009 RS485 Modbus decoder for PulseView/libsigrokdecode.
##
## Stack this decoder above the built-in UART decoder. The firmware poller
## uses Modbus RTU function 0x03 over the configured RS485 line.
##

import sigrokdecode as srd

try:
    from .wow_modbus import (describe_frame_variants, describe_register_variants,
...
class Decoder(srd.Decoder):
    api_version = 3
    id = 'wow_modbus'
    name = 'WOW Modbus {}'.format(DECODER_VERSION)
    longname = 'WOW / JK 009 RS485 Modbus {}'.format(DECODER_VERSION)
    desc = 'WOW / JK 009 BMS RS485 Modbus frames with PACE-compatible register layout.'
    license = 'gplv2+'
    inputs = ['uart']
    outputs = ['wow_modbus']
    ...
    annotation_rows = (
        ('frames', 'WOW Modbus {}: Frames'.format(DECODER_VERSION), (Ann.FRAME,)),
        ('fields', 'WOW Modbus {}: Fields'.format(DECODER_VERSION), (Ann.FIELD, Ann.CHECKSUM)),
        ('registers', 'WOW Modbus {}: Registers'.format(DECODER_VERSION), (Ann.REGISTER,)),
        ('decoded-values', 'WOW Modbus {}: Decoded'.format(DECODER_VERSION), (Ann.DECODED,)),
        ('warnings', 'WOW Modbus {}: Warnings'.format(DECODER_VERSION), (Ann.WARNING,)),
    )
    ...
    def maybe_flush_on_gap(self, rxtx, ss):
        ...
        if gap_samples > 0 and (ss - last_es) > gap_samples:
            ...
            self.put_ann(start, last_es, Ann.WARNING,
                         ['Incomplete WOW/Modbus frame before idle gap: {}'.format(hex_bytes(self.buf[rxtx])),
                          'Incomplete Modbus'])
            self.reset_direction(rxtx)
    ...
    def finish_frame(self, rxtx, es):
        ...
        try:
            ...
            self.put_ann(ss, es, Ann.WARNING,
                         ['Invalid WOW/Modbus frame: {} [{}]'.format(exc, hex_bytes(raw)),
                          'Invalid WOW', 'Invalid'])
            self.reset_direction(rxtx)
            return
    ...
    def decode(self, ss, es, data):
        ...
        if len(self.buf[rxtx]) > 260:
            self.put_ann(self.pos[rxtx][0][0], es, Ann.WARNING,
                         ['WOW/Modbus frame too long, dropping', 'Too long'])
            self.reset_direction(rxtx)
