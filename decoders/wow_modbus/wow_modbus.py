##
## WOW / JK 009 RS485 Modbus frame helpers.
##
## Kept dependency-free so the parser can be unit-tested without PulseView.
## JK profile 009 uses the same field-tested Modbus register layout as the
## PACE-compatible V1.3 map used by the firmware poller.
##
...
def frame_summary(frame, direction=''):
    ...
    if typ == 'request':
        return '{}WOW Modbus req slave=0x{:02X} func=0x{:02X} start=0x{:04X} count={} crc={}'.format(
    ...
    if typ == 'response':
        ...
        return '{}WOW Modbus rsp slave=0x{:02X} func=0x{:02X}{} bytes={} crc={}'.format(
            prefix, frame['slave'], frame['func'], reg_text, frame.get('byte_count', 0), crc_text)
    if typ == 'exception':
        return '{}WOW Modbus exception slave=0x{:02X} func=0x{:02X} code=0x{:02X} crc={}'.format(
            prefix, frame['slave'], frame['func'], frame['exception_code'], crc_text)
    return '{}WOW Modbus frame crc={}'.format(prefix, crc_text)
