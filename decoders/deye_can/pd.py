##
## Deye-compatible CAN protocol decoder for PulseView/libsigrokdecode.
##
## This decoder is standalone on a logic CAN RX signal. It reuses the built-in
## CAN decoder and adds Deye-specific annotations for decoded frames. It can
...
try:
    from .deye_can import FRAME_NAMES, VERSION, describe_packet, format_data, frame_summary
except Exception:
    from deye_can import FRAME_NAMES, VERSION, describe_packet, format_data, frame_summary
...
class Ann:
    DEYE_FRAME = len(CanDecoder.annotations) if CanDecoder is not None else 0
    DEYE_PAYLOAD = DEYE_FRAME + 1
    DEYE_DECODED = DEYE_FRAME + 2
    DEYE_WARNING = DEYE_FRAME + 3
...
class Decoder(BaseDecoder):
    api_version = 3
    id = 'deye_can'
    name = 'Deye CAN {}'.format(VERSION)
    longname = 'Deye-compatible low-voltage CAN {}'.format(VERSION)
    desc = 'Deye/JK low-voltage BMS frames over Classic CAN.'
    license = 'gplv2+'
    inputs = ['logic']
    outputs = ['deye_can']
    ...
    if CanDecoder is not None:
        annotations = CanDecoder.annotations + (
            ('deye-frame', 'Deye frame'),
            ('deye-payload', 'Deye payload'),
            ('deye-decoded', 'Deye decoded value'),
            ('deye-warning', 'Deye warning'),
        )
        annotation_rows = CanDecoder.annotation_rows + (
            ('deye-frames', 'Deye Frames', (Ann.DEYE_FRAME,)),
            ('deye-payloads', 'Deye Payload', (Ann.DEYE_PAYLOAD,)),
            ('deye-decoded-values', 'Deye Decoded', (Ann.DEYE_DECODED,)),
            ('deye-warnings', 'Deye Warnings', (Ann.DEYE_WARNING,)),
        )
    else:
        annotations = (
            ('deye-warning', 'Deye warning'),
    ...
    def put_deye_ann(self, ss, es, ann, texts):
    ...
    def putpy(self, data):
        ...
        if rtr_type not in (None, 'Data', 'data', 0):
            self.put_deye_ann(self.ss_packet, self.es_packet, Ann.DEYE_WARNING,
                              ['Remote frame 0x{:03X} ignored'.format(can_id), 'RTR ignored'])
            return

        packet = (frame_type, can_id, rtr_type, dlc, payload)
        self.put_deye_ann(self.ss_packet, self.es_packet, Ann.DEYE_FRAME,
                          [frame_summary(packet)])
        self.put_deye_ann(self.ss_packet, self.es_packet, Ann.DEYE_PAYLOAD,
                          [format_data(payload), 'DATA'])
        self.put_deye_ann(self.ss_packet, self.es_packet, Ann.DEYE_DECODED,
