##
## Sofar-compatible CAN protocol decoder for PulseView/libsigrokdecode.
##
## This decoder is standalone on a logic CAN RX signal. It reuses the built-in
## CAN decoder and adds Sofar-specific annotations for decoded frames. It can
...
try:
    from .sofar_can import FRAME_NAMES, VERSION, describe_packet, format_data, frame_summary
except Exception:
    from sofar_can import FRAME_NAMES, VERSION, describe_packet, format_data, frame_summary
...
class Ann:
    SOFAR_FRAME = len(CanDecoder.annotations) if CanDecoder is not None else 0
    SOFAR_PAYLOAD = SOFAR_FRAME + 1
    SOFAR_DECODED = SOFAR_FRAME + 2
    SOFAR_WARNING = SOFAR_FRAME + 3
...
class Decoder(BaseDecoder):
    api_version = 3
    id = 'sofar_can'
    name = 'Sofar CAN {}'.format(VERSION)
    longname = 'Sofar-compatible CAN {}'.format(VERSION)
    desc = 'Sofar-compatible low-voltage BMS frames over Classic CAN.'
    license = 'gplv2+'
    inputs = ['logic']
    outputs = ['sofar_can']
    ...
    if CanDecoder is not None:
        annotations = CanDecoder.annotations + (
            ('sofar-frame', 'Sofar frame'),
            ('sofar-payload', 'Sofar payload'),
            ('sofar-decoded', 'Sofar decoded value'),
            ('sofar-warning', 'Sofar warning'),
        )
        annotation_rows = CanDecoder.annotation_rows + (
            ('sofar-frames', 'Sofar Frames', (Ann.SOFAR_FRAME,)),
            ('sofar-payloads', 'Sofar Payload', (Ann.SOFAR_PAYLOAD,)),
            ('sofar-decoded-values', 'Sofar Decoded', (Ann.SOFAR_DECODED,)),
            ('sofar-warnings', 'Sofar Warnings', (Ann.SOFAR_WARNING,)),
        )
    else:
        annotations = (
            ('sofar-warning', 'Sofar warning'),
    ...
    def put_sofar_ann(self, ss, es, ann, texts):
    ...
    def putpy(self, data):
        ...
        if rtr_type not in (None, 'Data', 'data', 0):
            self.put_sofar_ann(self.ss_packet, self.es_packet, Ann.SOFAR_WARNING,
                               ['Remote frame 0x{:03X} ignored'.format(can_id), 'RTR ignored'])
            return

        packet = (frame_type, can_id, rtr_type, dlc, payload)
        self.put_sofar_ann(self.ss_packet, self.es_packet, Ann.SOFAR_FRAME,
                           [frame_summary(packet)])
        self.put_sofar_ann(self.ss_packet, self.es_packet, Ann.SOFAR_PAYLOAD,
                           [format_data(payload), 'DATA'])
        self.put_sofar_ann(self.ss_packet, self.es_packet, Ann.SOFAR_DECODED,
