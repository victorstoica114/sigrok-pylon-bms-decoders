##
## GoodWe-compatible CAN protocol decoder for PulseView/libsigrokdecode.
##
## This decoder is standalone on a logic CAN RX signal. It reuses the built-in
## CAN decoder and adds GoodWe-specific annotations for decoded frames. It can
...
try:
    from .goodwe_can import FRAME_NAMES, VERSION, describe_packet, format_data, frame_summary
except Exception:
    from goodwe_can import FRAME_NAMES, VERSION, describe_packet, format_data, frame_summary
...
class Ann:
    GOODWE_FRAME = len(CanDecoder.annotations) if CanDecoder is not None else 0
    GOODWE_PAYLOAD = GOODWE_FRAME + 1
    GOODWE_DECODED = GOODWE_FRAME + 2
    GOODWE_WARNING = GOODWE_FRAME + 3
...
class Decoder(BaseDecoder):
    api_version = 3
    id = 'goodwe_can'
    name = 'GoodWe CAN {}'.format(VERSION)
    longname = 'GoodWe-compatible low-voltage CAN {}'.format(VERSION)
    desc = 'GoodWe low-voltage BMS frames over Classic CAN.'
    license = 'gplv2+'
    inputs = ['logic']
    outputs = ['goodwe_can']
    ...
    if CanDecoder is not None:
        annotations = CanDecoder.annotations + (
            ('goodwe-frame', 'GoodWe frame'),
            ('goodwe-payload', 'GoodWe payload'),
            ('goodwe-decoded', 'GoodWe decoded value'),
            ('goodwe-warning', 'GoodWe warning'),
        )
        annotation_rows = CanDecoder.annotation_rows + (
            ('goodwe-frames', 'GoodWe Frames', (Ann.GOODWE_FRAME,)),
            ('goodwe-payloads', 'GoodWe Payload', (Ann.GOODWE_PAYLOAD,)),
            ('goodwe-decoded-values', 'GoodWe Decoded', (Ann.GOODWE_DECODED,)),
            ('goodwe-warnings', 'GoodWe Warnings', (Ann.GOODWE_WARNING,)),
        )
    else:
        annotations = (
            ('goodwe-warning', 'GoodWe warning'),
    ...
    def put_goodwe_ann(self, ss, es, ann, texts):
    ...
    def putpy(self, data):
        ...
        if rtr_type not in (None, 'Data', 'data', 0):
            self.put_goodwe_ann(self.ss_packet, self.es_packet, Ann.GOODWE_WARNING,
                                ['Remote frame 0x{:03X} ignored'.format(can_id), 'RTR ignored'])
            return

        packet = (frame_type, can_id, rtr_type, dlc, payload)
        self.put_goodwe_ann(self.ss_packet, self.es_packet, Ann.GOODWE_FRAME,
                            [frame_summary(packet)])
        self.put_goodwe_ann(self.ss_packet, self.es_packet, Ann.GOODWE_PAYLOAD,
                            [format_data(payload), 'DATA'])
        self.put_goodwe_ann(self.ss_packet, self.es_packet, Ann.GOODWE_DECODED,
