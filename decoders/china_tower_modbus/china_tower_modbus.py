##
## China Tower / JK 008 RS485 Modbus RTU frame helpers.
##
...
REG_PACK_VOLTAGE_10MV = 0x0000
REG_CELL_COUNT = 0x0001
REG_SOC_PCT = 0x0002
REG_TEMP1_C = 0x0006
REG_TEMP2_C = 0x0007
REG_MOS_TEMP_C = 0x0008
REG_CELL01_MV = 0x0009
REG_CELL16_MV = 0x0018
CELL_COUNT = 16
REG_WARNING_FLAGS = 0x0019
REG_PROTECTION_FLAGS = 0x001A
REG_STATUS_FLAGS = 0x001B
REG_TEMP_DUP1_C = 0x001C
REG_TEMP_DUP2_C = 0x001D
...
POLL_BLOCKS = (
    (REG_PACK_VOLTAGE_10MV, 0x000D),
    (REG_CELL01_MV, CELL_COUNT),
    (REG_WARNING_FLAGS, 0x0003),
)
...
REGISTER_NAMES = {
    ...
    REG_SOC_PCT: 'soc',
    0x0003: 'runtime_raw_0003',
    0x0004: 'runtime_raw_0004',
    0x0005: 'runtime_raw_0005',
    REG_TEMP1_C: 'temp1',
    REG_TEMP2_C: 'temp2',
    REG_MOS_TEMP_C: 'mos_temp',
    REG_WARNING_FLAGS: 'warning_flags',
    REG_PROTECTION_FLAGS: 'protection_flags',
    REG_STATUS_FLAGS: 'status_flags',
    REG_TEMP_DUP1_C: 'temp_dup1',
    REG_TEMP_DUP2_C: 'temp_dup2',
}
...
def describe_register(addr, value):
    ...
    if addr in (REG_TEMP1_C, REG_TEMP2_C, REG_MOS_TEMP_C, REG_TEMP_DUP1_C, REG_TEMP_DUP2_C):
        temp = temp_c_from_word(value)
        label = register_name(addr)
        if temp is None:
            return '{}_raw=0x{:04X}'.format(label, value)
        return '{}={}C'.format(label, temp)
...
def summarize_runtime(registers):
    ...
    if REG_PACK_VOLTAGE_10MV in values:
        parts.append(describe_register(REG_PACK_VOLTAGE_10MV, values[REG_PACK_VOLTAGE_10MV]))
    if REG_CELL_COUNT in values:
        parts.append(describe_register(REG_CELL_COUNT, values[REG_CELL_COUNT]))
    if REG_SOC_PCT in values:
        parts.append('SOC={}%'.format(decode_packed_pct(values[REG_SOC_PCT])))
    for addr in (REG_TEMP1_C, REG_TEMP2_C, REG_MOS_TEMP_C):
        if addr in values:
            parts.append(describe_register(addr, values[addr]))
    for addr in (REG_WARNING_FLAGS, REG_PROTECTION_FLAGS, REG_STATUS_FLAGS):
        if addr in values:
            parts.append(describe_register(addr, values[addr]))
...
def describe_registers(registers):
    ...
    if start is not None and start == REG_CELL01_MV and cell_text:
        return cell_text
    if runtime:
        if cell_text and start is not None and start <= REG_CELL01_MV <= end:
            runtime.append(cell_text)
        return ', '.join(runtime[:8])
    if cell_text:
        return cell_text
...
def frame_summary(frame, direction=''):
    ...
    if typ == 'request':
        return '{}China Tower Modbus req slave=0x{:02X} func=0x{:02X} start=0x{:04X} count={} crc={}'.format(
    ...
    if typ == 'response':
        ...
        return '{}China Tower Modbus rsp slave=0x{:02X} func=0x{:02X}{} bytes={} crc={}'.format(
            prefix, frame['slave'], frame['func'], reg_text, frame.get('byte_count', 0), crc_text)
    if typ == 'exception':
        return '{}China Tower Modbus exception slave=0x{:02X} func=0x{:02X} code=0x{:02X} crc={}'.format(
            prefix, frame['slave'], frame['func'], frame['exception_code'], crc_text)
    return '{}China Tower Modbus frame crc={}'.format(prefix, crc_text)
