##
## PACE RS485 Modbus V1.3 decoder for PulseView/libsigrokdecode.
##
...
try:
    from .pace_modbus import (describe_frame_variants, describe_register_variants,
...
class Decoder(srd.Decoder):
    api_version = 3
    id = 'pace_modbus'
    name = 'PACE Modbus {}'.format(DECODER_VERSION)
    longname = 'PACE RS485 Modbus V1.3 {}'.format(DECODER_VERSION)
    desc = 'PACE BMS RS485 Modbus V1.3 frames.'
    license = 'gplv2+'
    inputs = ['uart']
    outputs = ['pace_modbus']
    ...
    annotation_rows = (
        ('frames', 'PACE Modbus {}: Frames'.format(DECODER_VERSION), (Ann.FRAME,)),
        ('fields', 'PACE Modbus {}: Fields'.format(DECODER_VERSION), (Ann.FIELD, Ann.CHECKSUM)),
        ('registers', 'PACE Modbus {}: Registers'.format(DECODER_VERSION), (Ann.REGISTER,)),
        ('decoded-values', 'PACE Modbus {}: Decoded'.format(DECODER_VERSION), (Ann.DECODED,)),
        ('warnings', 'PACE Modbus {}: Warnings'.format(DECODER_VERSION), (Ann.WARNING,)),
    )
    ...
    def maybe_flush_on_gap(self, rxtx, ss):
        ...
        if gap_samples > 0 and (ss - last_es) > gap_samples:
            ...
            self.put_ann(start, last_es, Ann.WARNING,
                         ['Incomplete PACE/Modbus frame before idle gap: {}'.format(hex_bytes(self.buf[rxtx])),
                          'Incomplete Modbus'])
            self.reset_direction(rxtx)
    ...
    def finish_frame(self, rxtx, es):
        ...
        try:
            ...
            self.put_ann(ss, es, Ann.WARNING,
                         ['Invalid PACE/Modbus frame: {} [{}]'.format(exc, hex_bytes(raw)),
                          'Invalid PACE', 'Invalid'])
            self.reset_direction(rxtx)
            return
    ...
    def decode(self, ss, es, data):
        ...
        if len(self.buf[rxtx]) > 260:
            self.put_ann(self.pos[rxtx][0][0], es, Ann.WARNING,
                         ['PACE/Modbus frame too long, dropping', 'Too long'])
            self.reset_direction(rxtx)
